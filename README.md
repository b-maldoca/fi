...
*   **Three Comparative Simulation Methods**: Compare outcomes side-by-side using:
    ...
    3. **Parametric Monte Carlo**: Stochastic lognormal return generator (with Ito drift correction) with cross-asset shock coupling (`US`/`Non-US` $\rho=0.75$, `Gold` $\rho=0.08$ — the 1922–2025 annual US/gold correlation — and `Bitcoin` $\rho=\text{btc\_equity\_corr}$, default `0.50`) and independent annual inflation stream (`seed + 10_000`). Stock means, volatilities and gold defaults are calibrated at runtime to the 1922–2025 CHF history via `historic_lognormal_params` (arithmetic means US ≈ 11.3%, ex-US ≈ 8.6%, gold ≈ 4.7%; log-vols equity ≈ 21%, gold ≈ 15.3%); CHF cash (1%) and inflation (2.5% ± 1%) stay forward-looking. Every input's help text shows its historic value.
