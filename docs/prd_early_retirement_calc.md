...
*   **Monte Carlo & Simulation Parameters**:
    *   Nominal Mean Returns (%) and Volatilities (%) for asset classes, plus Inflation Mean (%) and Inflation Volatility (%) for Monte Carlo mode. The section caption quotes the **data-derived** 1922–2025 CAGRs (US equities in USD and CHF, ex-US in CHF) computed from `src/historic_returns.py` at runtime rather than hardcoded figures. **Defaults (calibrated vs. assumed):** the US and ex-US stock means, equity volatility, gold mean and gold volatility defaults are **derived at runtime from the 1922–2025 CHF history** via `historic_lognormal_params` (arithmetic mean of annual simple returns and standard deviation of annual *log* returns — exactly the parameters `generate_monte_carlo_returns` consumes): US mean ≈ 11.3%, ex-US mean ≈ 8.6%, equity vol ≈ 21.0% (average of US 22.2% and ex-US 19.9%), gold mean ≈ 4.7%, gold vol ≈ 15.3%. CHF cash (1%) and inflation (2.5% ± 1%) remain **forward-looking assumptions** (today's near-zero savings rates and the SNB 0–2% target; the historic 2.5% cash mean and 1.8% ± 3.6% inflation are dominated by earlier regimes). Every Monte Carlo input's help text shows its historic counterpart. App default, Static spending: success 52.4% (old 15% vol, 7%/6% means) → 36.6% (calibrated vol, 7%/6%) → 75.9% (fully calibrated); Vanguard Dynamic 65.7% → 45.0% → 85.2%.
