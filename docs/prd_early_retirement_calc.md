...
*   **Monte Carlo & Simulation Parameters**:
    *   Nominal Mean Returns (%) and Volatilities (%) for asset classes, plus Inflation Mean (%) and Inflation Volatility (%) for Monte Carlo mode. The section caption quotes the **data-derived** 1922–2025 CAGRs (US equities in USD and CHF, ex-US in CHF) computed from `src/historic_returns.py` at runtime rather than hardcoded figures. **Defaults (calibrated vs. assumed):** the US and ex-US stock means, equity volatility, gold mean, gold volatility, inflation mean and inflation volatility defaults are **derived at runtime from the 1922–2025 CHF history** (`historic_lognormal_params` for lognormal assets — arithmetic mean of annual simple returns and standard deviation of annual *log* returns — and sample mean/std for `HISTORIC_SWISS_INFLATION`): US mean ≈ 11.3%, ex-US mean ≈ 8.6%, equity vol ≈ 21.0% (average of US 22.2% and ex-US 19.9%), gold mean ≈ 4.7%, gold vol ≈ 15.3%, inflation mean ≈ 1.8%, inflation volatility ≈ 3.6%. Only CHF cash (1%) remains a **forward-looking assumption** (today's near-zero savings rates and Year 0 taxable interest; the historic 2.5% cash mean is dominated by earlier regimes). Every Monte Carlo input's help text shows its historic counterpart. App default, Static spending: success 52.4% (old 15% vol, 7%/6% means, 2.5% ± 1% inflation) → 36.6% (calibrated vol, 7%/6%) → 75.9% (calibrated asset means & vols, 2.5% ± 1% inflation) → 81.4% (fully calibrated including 1.8% ± 3.6% inflation); Vanguard Dynamic 65.7% → 45.0% → 85.2% → 89.7%.
...
*   **Empirical Datasets Ingested (`data/`)**:
    ...
    *   **Swiss Retail Cash Rate (`data/ch_cash_rate.csv`)**: Annual CHF cash yield (`1900–2025`, `retail_rate`) = **SNB interest rate on private-client savings deposits** (data.snb.ch cube `zikrepro`, `D1=S1`, annual mean of monthly values) from **1933**, and the **Jordà–Schularick–Taylor R6** Swiss `bill_rate` for 1900–1932 (identical to the savings rate on the 1933–1968 overlap, 0.007pp mean abs diff). This replaces the former wholesale proxy (JST money-market `bill_rate` from 1969 + SNB SARON, floored at 0%), which had the same average (2.30% vs 2.32% over 1933–2025) but far higher volatility (e.g. 1989: 9.7% vs 3.45%). `MONTHLY_CASH_CHF`, `HISTORIC_RETURNS_CASH_CHF`, 1922–2025: `2.49%` nominal CAGR, `1.42%` vol, `+0.74%` real after `1.74%` Swiss CPI. The wholesale series is retained in the CSV (`wholesale_rate`) for reference only.
    *   **US Inflation (`data/us_cpi.csv`)**: Monthly US CPI-U — **FRED `CPIAUCNS`** from 1913, legacy swr-calculator leg for 1871–1912 — fetched by `scripts/fetch_source_data.py`. Build-time only: used to derive `HISTORIC_REAL_CHF_APPRECIATION` $= 1 - \frac{1 + g_{\text{FX}}}{1 + g_{\text{PPP}}}$ (geometric 1922–2025 drifts, currently `0.0071`) instead of a hand-maintained constant.
...
*   **Withdrawal Rate Chart**: Percentile/extrema chart showing the annual withdrawal rate over time, computed as `(Living Expenses + Taxes) / Beginning-of-Year Net Worth`, where the beginning-of-year net worth is reconstructed as end-of-year net worth plus that year's outflows (dynamically capped at 25% max for readability). In **Historic Backtesting**, the `0th` percentile (lowest withdrawal rate) is labeled **Best Cohort (Min)** and the `100th` percentile (highest withdrawal rate) is labeled **Worst Cohort (Max)**, with color coding inverted (`colors[::-1]`) relative to the Net Worth chart so `Best` (`purple`) and `Worst` (`crimson`) remain visually consistent.
