...
5. **Rebalancing Check (Monthly / Quarterly / Threshold / Month 11 for Yearly & Cash Tent)**:
    ...
    * **Smart Cash Buffer**: If `enable_smart_selling` is enabled, rebalancing is skipped for any run currently in a market downturn (`current_nw_after_tax < initial_net_worth_after_tax * inflation_factors`).
    * **After-tax watermark (`_deferred_withdrawal_tax`)**: every watermark comparison (Smart Cash Buffer, Dynamic Floor & Ceiling, `below_watermark`) uses net worth minus the capital withdrawal tax still owed on the pension balances. `_deferred_withdrawal_tax(pillar_2, pillar_3a, from_age, bracket_factors, config)` replays the liquidation schedule from `from_age` on current balances (no growth): before 65 at most one non-empty 3a account per year (first account with `60 + i <= age`), then all remaining 3a accounts plus Pillar 2 as one event at 65; each event is taxed with `_indexed_tax(calculate_capital_withdrawal_tax, …)`. Inside the loop `from_age = current_age + 1` (this year's month-0 liquidation already ran); the starting base `initial_net_worth_after_tax` uses `from_age = start_age` and unit bracket factors. Vanguard Dynamic's `target_exp` and the success criterion still use gross net worth.
...
*   **Parametric Monte Carlo Mode (`generate_monte_carlo_returns`, `generate_monte_carlo_inflation`)**: Generates an asset return matrix of `shape=(num_runs, duration_months, 5)` seeded by `random_seed` using a lognormal model with Ito drift correction ($\text{drift} = \ln(1 + R) - 0.5\sigma^2$) and correlated monthly shocks (`US`/`Non-US` $\rho=0.75$, `Gold` $\rho=0.08$, `Bitcoin` $\rho=\text{btc\_equity\_corr}$) for US Stocks, Non-US Stocks, Gold, and Bitcoin, constant geometric monthly return $(1 + R_{\text{cash}})^{1/12} - 1$ for CHF Cash, and independent normally distributed annual inflation `generate_monte_carlo_inflation` seeded at `random_seed + 10_000`. **Parameterisation & calibration (`historic_lognormal_params`)**: `ann_ret` is the *arithmetic* mean of annual simple returns ($E[1+R] = 1 + \text{ann\_ret}$) and `ann_vol` is the standard deviation of annual *log* returns; `historic_lognormal_params(annual_returns) -> (mean, log_vol)` is the exact inverse and is used by the UI to default Equities Volatility (average of US and ex-US log-vol, ≈ 21.0%), Gold Mean (≈ 4.7%) and Gold Volatility (≈ 15.3%) to the 1922–2025 CHF history. Stock, cash and inflation means stay conservative forward-looking defaults (US 7%, ex-US 6%, cash 1%, inflation 2.5% ± 1%); their historic values are shown in each input's help text.
...
*   `below_watermark`: Boolean mask indicating runs whose *after-tax* net worth is below the inflation-adjusted after-tax starting watermark `shape=(duration_years, num_runs)`
*   `initial_net_worth`: Float scalar starting net worth (gross; used by the success criterion)
*   `initial_net_worth_after_tax`: Float scalar starting net worth minus the deferred capital withdrawal tax on the initial pension balances (watermark base)
