...
*   **Historic Bootstrapping Mode (`generate_bootstrapped_data`)**: Generates $N$ simulation runs of length `duration_years` seeded by `random_seed` by jointly block-sampling contiguous 5-year historical windows (`block_size_years = 5`) with replacement for US Equities (CHF), Non-US Equities (CHF), and Swiss CPI inflation, paired with exact 1% geometric nominal APY for CHF Cash and vectorized Ito-corrected lognormal Gold/Bitcoin returns. Sampling 5-year contiguous blocks preserves multi-year market regimes (crashes followed by recoveries, or multi-year inflation waves) while preventing unrealistic single-year regime jumps.
