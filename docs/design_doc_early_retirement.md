...
*   **Validation Rules**: `num_runs > 0`, `duration_years > 0`, `start_age >= 0`, `tent_duration_years >= 0`, `inflation_std >= 0.0`, `seed >= 0`, `real_chf_appreciation < 1.0`, `-1.0 <= btc_equity_corr <= 1.0`, `bracket_indexation >= 0.0`, `spending_strategy in VALID_SPENDING_STRATEGIES`, `rebalance_strategy in VALID_REBALANCE_STRATEGIES`, individual asset allocations $\ge 0.0$ and $\sum \text{allocations} = 1.0$, non-negative initial asset balances (`initial_liquid_wealth`, `initial_pillar_2`, `initial_pillar_3a_accounts`), non-negative expenses/income/yields (`annual_base_expenses`, `monthly_ahv_pension`, `dividend_yield`), non-negative tax multipliers (`cantonal_multiplier`, `municipal_multiplier`), non-negative rebalancing thresholds, and non-negative spending strategy parameters (`vanguard_target_rate`, `vanguard_floor_pct`, `vanguard_ceiling_pct`, `dynamic_expense_floor_pct`, `dynamic_expense_ceiling_pct`).
*   **Normalization**: `initial_pillar_3a_accounts` defaults to an empty list when `None`, and the legacy `enable_dynamic_expenses=True` flag coerces `spending_strategy` from `"Static"` to `"Dynamic (Floor & Ceiling)"` before strategy validation runs.
*   **Demographics**: Retirement age (`start_age`), simulation duration (`duration_years`).
*   **Economics & Taxes**: Inflation mean & volatility (for Monte Carlo), dividend yield, Year 0 cash interest rate (`cash_rate`, default `0.01`), Bitcoin–equity monthly log-return correlation (`btc_equity_corr`, default `0.50`), cantonal tax multiplier (`0.95` for Canton Zurich 2026), municipal tax multiplier (Steuerfuss, e.g., `1.19` for Zurich City), **Real CHF Appreciation beyond PPP** (`real_chf_appreciation`, default `0.0` = Relative PPP neutrality; `+0.0068` = raw 1922–2025 historical real CHF appreciation), and **Tax Bracket Indexation** (`bracket_indexation`, default `1.0` = full CPI indexation per Art. 39 DBG / § 48 StG ZH).
...
*   **Bracket Table Vintages**: `FEDERAL_INCOME_BRACKETS` (~2024, Single tariff), `ZURICH_INCOME_BRACKETS` (~2024, Single, base rate), `ZURICH_WEALTH_BRACKETS` (2025, Single, base rate), and the AHV non-worker table (530 CHF min / 26,500 CHF max, unchanged for 2026). These are the *base-year* tables; the simulation engine indexes them forward each year at runtime (see **Bracket Indexation** below) rather than storing pre-inflated copies. The cantonal/municipal multipliers are 2026 values supplied by `app.py`.
...
*   **Empirical Source Datasets (`data/`)**:
    ...
    *   `ch_cash_rate.csv`: Empirical Swiss short-term interest rate (`1900–2025`), splicing the **Jordà–Schularick–Taylor Macrohistory Database R6** (`bill_rate` for Switzerland, 1900–2020) with **SNB policy / SARON rates** (2021–2025), floored at `0.0%` (`retail_rate`) to reflect Swiss retail savings deposit pass-through during the 2015–2022 SNB negative-interest-rate era (`MONTHLY_CASH_CHF`, `HISTORIC_RETURNS_CASH_CHF`, `2.46%` nominal CAGR, `1.88%` vol).
*   **Currency / PPP Adjustment (`_apply_fx_ppp_adjustment`, `HISTORIC_REAL_CHF_APPRECIATION = 0.0068`)**: Over 1922–2025, `USD/CHF` drifted by `-1.78%/yr` while the Swiss-vs-US CPI inflation differential implied `-1.09%/yr` under Relative Purchasing Power Parity (PPP), leaving a `-0.68%/yr` historical real FX drag on foreign-priced assets (`+0.68%/yr` real CHF appreciation beyond PPP). When `real_chf_appreciation` is passed (UI default `0.0`), foreign-priced sleeves (`ASSET_US_STOCKS`, `ASSET_NON_US_STOCKS`, `ASSET_GOLD`) are multiplicatively scaled each month by $\text{month\_adj} = \left(\frac{1 - \text{real\_chf\_appreciation}}{1 - 0.0068}\right)^{1/12}$ via $r_m^{\text{adj}} = (1 + r_m)\cdot\text{month\_adj} - 1$.
...
class SimConfig:
    ...
    real_chf_appreciation: float = 0.0 # Expected real CHF appreciation beyond PPP (0.0 = PPP neutrality; 0.0068 = raw 1922-2025 history)
...
*   **Sidebar**: All inputs categorized into 11 dedicated sections: **Demographics**, **Success Criteria**, **Initial Assets (CHF)**, **Target Asset Allocation (%)**, **Rebalancing Strategy**, **Withdrawal Strategy**, **Spending Strategy**, **Income & Yield**, **Monte Carlo Parameters** (including **Bitcoin–Equity Correlation ($\rho$)** slider default `0.50`, **Stationary Bootstrap Mean Block Length (Years)** slider default `5`, run counts, and **Random Seed**), **Currency / PPP Assumption** (**Real CHF Appreciation beyond PPP (%/yr)** slider, default `0.00%`), and **Zurich Tax Location** (Municipal Multiplier and **Tax Bracket Indexation (% of CPI)**, default `100`).
*   **Main Panel**: Displays results across three side-by-side columns for direct comparative analysis: **Historic Backtesting**, **Historic Bootstrapping**, and **Monte Carlo**. Each column contains mouse-over header tooltips and:
    ...
    *   **Metrics**: Probability of Success (based on selected success criteria, with **Historic Backtesting** displaying a caption and tooltip with $N_{\text{cohorts}}$, **Effective Sample Size ($N_{\text{eff}} = T / D$)**, and the **90% Wilson confidence interval**), Avg Years Below Start NW, Median Ending Net Worth (Real & Nominal), Median Total Withdrawals (Real & Nominal), Pre-AHV Outflow (< Age 65), and Post-65 Outflow (Age 65+).
